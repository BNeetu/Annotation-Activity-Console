...
app.get("/api/tasks", (req, res) => {
  ...
  // Per the brief: "Do not change its behavior." Defaults restored to the
  // exact spec values (200ms / 1200ms on every 3rd page). The env var
  // override still exists purely as an opt-in convenience for local/personal
  // use -- with no env vars set, behavior is byte-for-byte the original.
  const FAST_MS = Number(process.env.MOCK_FAST_DELAY_MS) || 200;
  const SLOW_MS = Number(process.env.MOCK_SLOW_DELAY_MS) || 1200;
  const delay = page % 3 === 0 ? SLOW_MS : FAST_MS;
  setTimeout(() => {
